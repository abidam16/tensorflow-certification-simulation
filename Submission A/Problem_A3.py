...
class customCallback(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs={}):
        if logs.get('acc') is not None and logs.get('acc') > 0.97 and logs.get('val_acc') > 0.97:
            print("\nAll acc > 97%!")
...
def solution_A3():
    ...
    last_layer = pre_trained_model.get_layer('mixed6').output
    ...
    model.fit(train_generator, epochs=30, validation_data=validation_generator, callbacks=[callback])
