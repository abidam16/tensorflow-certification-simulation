...
class customCallback(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs={}):
        if logs.get('mae') is not None and logs.get('mae') < 0.15:
            print("\nMAE is less than 0.15")
...
def solution_A5():
    ...
    model=tf.keras.models.Sequential([
      ...
      tf.keras.layers.Conv1D(filters=32, kernel_size=3,
                             strides=1, activation='swish', padding='causal',
    ...
    model.fit(train_set, epochs=200, callbacks=[callback])
